...
- Read access to the MATEY model + XGC data directories used by the
  example (under `/global/cfs/...`), or your own substitutes.
...
The example tasks run MATEY's `basic_inference.py`.  On Perlmutter:

1. Check out MATEY at the location referenced by `amsc.py`'s
   `IRI_DEFAULTS['nersc']['app']['matey_dir']` (default
   `/global/u2/m/<you>/MATEY`).
2. Place `matey_wrapper.sh` (shipped in this directory) at
   `$MATEY_DIR/matey_wrapper.sh` and make it executable:

   ```sh
   cp matey_wrapper.sh "$MATEY_DIR/matey_wrapper.sh"
   chmod +x "$MATEY_DIR/matey_wrapper.sh"
   ```

3. **Edit the wrapper** for your layout — it hardcodes the MATEY env:

   ```sh
   module load conda
   . ~/MATEY/prep            # <- your MATEY env-prep script
   cd ~/MATEY/examples       # <- your MATEY examples dir
   ```

## Point `amsc.py` at your account and data

Edit the `IRI_DEFAULTS['nersc']` block in `amsc.py`:

- `account` — your Perlmutter GPU allocation (e.g. `m5290` or `amsc007_g`).
- `home_dir` — your `$HOME` on Perlmutter (used to build the wrapper path).
- `n_nodes`, `gpus_per_node`, `cores_per_node` — allocation shape.
- `app.matey_dir` / `app.matey_model_dir` / `app.matey_xgc_dir` — MATEY
  checkout + model checkpoint + XGC data paths on Perlmutter.

Only MATEY is enabled in the shipped config (GKEYLL is zeroed out); leave
that as-is for this use case.
