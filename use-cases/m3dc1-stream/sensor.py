...
class SensorStream(ABC):
    """Abstract sensor that emits one M3DC1 observation at a time.

    Implement read_one() for any real sensor, REST endpoint, Kafka consumer,
    shared-memory ring buffer, etc.  The default stream() calls read_one() in
    a loop; override it for push-based sources where data arrives on its own.
    """

    @abstractmethod
    async def read_one(self) -> dict[str, float]:
        """Return a single observation as a column-name → value mapping."""

    async def stream(self) -> AsyncIterator[dict[str, float]]:
        """Yield observations indefinitely. Override for push-based sensors."""
        while True:
